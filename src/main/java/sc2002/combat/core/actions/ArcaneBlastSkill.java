...
public class ArcaneBlastSkill implements ISpecialSkillAction {
    ...
    public void execute(Entity attacker, Entity ignored, BattleContext context) {
        context.getBoundary().onActionExecuted(attacker, "Arcane Blast", null);
        context.getBoundary().displayMessage(attacker.getName() + " unleashes a wave of magical energy!");
        ...
        for (Entity e : context.getEntities()) {
            ...
            if (e.isAlive()) {
                ...
                context.getBoundary().onDamageDealt(e, finalDamage, e.getHp(), !e.isAlive());

                if (!e.isAlive()) {
                    attacker.increaseBaseAttack(10);
                    context.getBoundary().displayMessage(attacker.getName() + " gains +10 ATK from the blast!");
                }
            }
        }
    }
